...
public static class LaunchComposer
{
        ...
        string target, string startDir, string options)
    {
        ...
        // Putting the ROM inside Exe makes vssh-dolphin.sh start without a game.
    ...
    public static bool IsVisualSshLaunch(string? target, string? options = null)
    {
        ...
        return hay.Contains("/vssh-", StringComparison.OrdinalIgnoreCase);
    }

    public static bool IsLegacyScript(string? target, string? options = null)
    {
        if (!IsVisualSshLaunch(target, options)) return false;
        return !DolphinInput.IsBound(((target ?? "") + " " + (options ?? "")).Replace('\\', '/'));
    }
