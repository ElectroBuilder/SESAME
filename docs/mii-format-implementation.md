...
The visual appearance editor supports the fields that have an unambiguous direct representation in both formats: name, gender, favourite colour, hair style, hair colour and eye colour. It shows a live vector preview, readable style names and colour swatches while retaining the exact emulator IDs internally. The preview is an editing aid, not a replacement for the emulator's renderer; it uses a stable SESAME illustration rather than claiming to reproduce every emulator asset. It preserves all other record bytes. Wii values follow the documented RFL bit layout; SwitchDB values follow its StoreData bit sequence and both checksums are regenerated. The remaining face controls (face shape, eyes, brows, nose, mouth, facial hair, glasses and mole) are not yet written because their cross-format mappings still require separate verification.
